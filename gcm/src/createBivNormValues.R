...
Temp <- seq(0, 6, by=temp_increment) 
...
trend_periods <- seq(1995,2085)
rcp_clim_trends <- function(climate_delta_dataframe,rcp_columns) {
    climate_trend_rcp <- cbind(trend_periods,
...
pr_cv_fut_clim_delta.rcp45.melt <- cbind(rep(trend_periods, times = 33),melt(t(pr_cv_fut_clim_delta)[,c(rcp45)]))
pr_cv_fut_clim_delta.rcp85.melt <- cbind(rep(trend_periods, times = 33),melt(t(pr_cv_fut_clim_delta)[,c(rcp85)]))
...
pr_fut_clim_delta.rcp45.melt <- cbind(rep(trend_periods, times = 33),melt(t(pr_fut_clim_delta)[,c(rcp45)]))
pr_fut_clim_delta.rcp85.melt <- cbind(rep(trend_periods, times = 33),melt(t(pr_fut_clim_delta)[,c(rcp85)]))
...
t_fut_clim_delta.rcp45.melt <- cbind(rep(trend_periods, times = 33),melt(t(t_fut_clim_delta)[,c(rcp45)]))
t_fut_clim_delta.rcp85.melt <- cbind(rep(trend_periods, times = 33),melt(t(t_fut_clim_delta)[,c(rcp85)]))
...
GCM_data <- inner_join(GCM_data, subset(pr_cv_fut_clim_delta.melt,r ==1), by=c("GCM","RCP","Year","period"))
...
hotelling_extremes <- function (extreme_point, hotelling_cr) {
  low <- Inf
  min <- 0
  for (i in 1:length(hotelling_cr[['Var1']])) {
    a<-(hotelling_cr[['Var1']][[i]]-min(hotelling_cr[['Var1']]))/(max(hotelling_cr[['Var1']])-min(hotelling_cr[['Var1']]))
    b<-(hotelling_cr[['Var2']][[i]]-min(hotelling_cr[['Var2']]))/(max(hotelling_cr[['Var2']])-min(hotelling_cr[['Var2']]))
    d<-dist(rbind(c(a,b),extreme_point))
    if(d < low){
      low <- d
      min <- i
    }
  }
  a_min <-hotelling_cr[['Var1']][[min]]
  b_min <-hotelling_cr[['Var2']][[min]]
  return (c(b_min,a_min))
}
...
df <- do.call(rbind.data.frame, norm_probs_normalize.dtdsd)
dbWriteTable(con,'ref_gcm_bivNormVal_dtdsd',df,append=FALSE, row.names=FALSE)
...
for(i in 1: length(unique(GCMs_dataframe_family$Year)))
{
  ...
  # hotelling[[i]] <- cset(dat=Years[[i]][c('DP','DT')], method="hotelling", alpha=0.05)
}
...
for(j in 1:length(unique(GCMs_dataframe_family$Year)))
{
  ...
  # mean_low <- hotelling_extremes(c(1,0),hotelling[[j]][['cr']])
  # mean_high <- hotelling_extremes(c(0,1),hotelling[[j]][['cr']])
  # 
  # norm_probs_mean_high <- dmvnorm(stress_test_grid,mean=as.numeric(mean_high),sigma=sigs[[j]])
  # norm_probs_mean_low <- dmvnorm(stress_test_grid,mean=as.numeric(mean_low),sigma=sigs[[j]])
  ...
  # norm_probs_normalize_mean_high.dtdp[[j]] <- cbind(stress_test_grid,norm_probs_mean_high/sum(norm_probs_mean_high))
  # colnames(norm_probs_normalize_mean_high.dtdp[[j]]) <- c("T_lev","P_lev","Biv_Norm_Prob")
  # norm_probs_normalize_mean_high.dtdp[[j]]$period<- GCMs_dataframe_family$Year[[j]]
  # 
  # norm_probs_normalize_mean_low.dtdp[[j]] <- cbind(stress_test_grid,norm_probs_mean_low/sum(norm_probs_mean_low))
  # colnames(norm_probs_normalize_mean_low.dtdp[[j]]) <- c("T_lev","P_lev","Biv_Norm_Prob")
  # norm_probs_normalize_mean_low.dtdp[[j]]$period<- GCMs_dataframe_family$Year[[j]]
}
saveRDS(norm_probs_normalize.dtdp, file= "../processed/biv_norm_values_dt-dp.RDS")
# saveRDS(norm_probs_normalize_mean_high.dtdp, file= "../processed/biv_norm_values_dt-dp_mean_high.RDS")
# saveRDS(norm_probs_normalize_mean_low.dtdp, file= "../processed/biv_norm_values_dt-dp_mean_low.RDS")

# write.csv(norm_probs_normalize_mean_low.dtdp,'../processed/biv_norm_vals_dt-dp_mean_low.csv')
# write.csv(norm_probs_normalize_mean_high.dtdp,'../processed/biv_norm_vals_dt-dp_mean_high.csv')
...
prob_plot <- my.filled.contour(x=SD, y=Temp, z=as.matrix(EvalData_stress_matrix.dtdsd),
   levels=mylevel, col=mycolors, plot.axes={
     points(y=plotRCP45$D_tas, x=plotRCP45$D_cv, pch=20, col='lightgreen');
     points(y=plotRCP45_family$DT, x=plotRCP45_family$DSD, pch=10, col='green3',cex=2);
     points(y=plotRCP85$D_tas, x=plotRCP85$D_cv, pch=20, col='tan');
     points(y=plotRCP85_family$DT, x=plotRCP85_family$DSD, pch=10, col='orange3',cex=2);
...
prob_plot <- my.filled.contour(x=Precip, y=Temp, z=as.matrix(EvalData_stress_matrix.dtdp),
   levels=mylevel, col=mycolors, plot.axes={
     ...
     points(y=plotRCP45_family$DT, x=plotRCP45_family$DP, pch=10, col='green3',cex=2);
     points(y=plotRCP85$D_tas, x=plotRCP85$D_pr, pch=20, col='tan');
     points(y=plotRCP85_family$DT, x=plotRCP85_family$DP, pch=10, col='orange3',cex=2);
   ...
   ylab="Change in Temperature (C)",xlim=c(-20, 30))
points(hotelling[[climate_period]][['cr']],col='red',pch=20)
print(prob_plot)
dev.off()
