...
sacAirpTempAll <- vector("list",length = 5)
for (i in 1:6) {
  sacAirpTempAll[[i]] <-
    warm(sacAirpTempHist, warmingType,  i-1)
}
remove(i)
#sacAirpTempAll[[1]]
```
...
inputsCorr <- dbGetQuery(con,
  paste('SELECT Time_Series AS dssInput, MAX_R2 AS r2, MAX_NAME as corrFlow',
        ...
        'WHERE MAX_NAME <> "SJWYT" AND MAX_NAME <> "SacWYT" AND MAX_NAME <> "No Mapping"'))
...
for (cc_idx in cc) {
  ...
  if (cc_idx %in% list(6,11,16)) {
    t<-2
  } else if (cc_idx %in%  list(2,7,12,17,21)) {
    t<-3
  } else if (cc_idx %in%  list(3,8,13,18,22)) {
    t<-4
  } else if (cc_idx %in%  list(4,9,14,19,21)) {
    t<-5
  } else if (cc_idx == 1) {
    t<-1
  } else {
  ...
  # Station Sim
  obs11stations <- stationRead(cc_idx,con,
  ...
  add9stations <- stationRead(cc_idx,con,
                              calLiteRunCount,modelYears,
  ...
  rim12stations <- stationRead(cc_idx,con,
                                calLiteRunCount,modelYears,
  ...
  wytResults <- wyTyping(cc_idx,
                    ...
                    con,calLiteRunCount,runName,modelYears,obs11stations,dbWrite=FALSE)
  ...
  # WYT
  dssInWYTyped <- wytMapping(inputsWYTSac,inputsWYTSJ,cc_idx,
                          con,calLiteRunCount,modelYears,runName)

  # DSS new
  dssInNew <- dssNew(calLiteRunCount,dssIn[1:modelYears*12,],dssInWYTyped,dssInQmapped)
